...
class GetAllData
{
    ...
    public static function execute()
    {
        try {
            ...
            $pageLimit = 100;
            $orderByColumn = request()->input('sort_by_col') ?? 'order';
            ...
            if (request()->has('search') && request()->input('search')) {
                ...
                $data = $data->where(function ($q) use ($searchKey) {
                    $q->where('project_id', 'like', '%' . $searchKey . '%');

                    $q->orWhere('module_name', 'like', '%' . $searchKey . '%');
                });
            }
            ...
            if (request()->has('with_management') && request()->input('with_management')) {

                $with['module_management'] = function ($query) {
