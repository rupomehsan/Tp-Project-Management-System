...
export default {
  ...
  created: function () {
    this.$watch(
      ...
      (newValue) => {
        if (newValue) {
          // If value is an array (your case)
          if (Array.isArray(newValue)) {
            this.social_media = newValue.map((doc) => ({
              name: doc.name,
              link: doc.link,
            }));
          }
          // If value has social_media property
          else if (newValue.social_media) {
            this.social_media = newValue.social_media.map((doc) => ({
              name: doc.name,
              link: doc.link,
            }));
          }
          // If value has links property (current component structure)
          else if (newValue.links && Array.isArray(newValue.links)) {
            this.social_media = newValue.links.map((doc) => ({
              name: doc.name,
              link: doc.link,
            }));
          }
