...
export default {
  ...
  methods: {
      ...
      if (this.item) {
        this.form_fields.forEach((field, index) => {
          Object.entries(this.item).forEach((value) => {
            ...
            if (field.name == "message" && value[0] == "message") {
              $("#message").summernote("code", this.item.message);
            }
          });
        });
      }
    ...
    get_all_users: async function () {
      const response = await axios.get("users?get_all=1");
      if (response.data.status == "success") {
        const userField = this.form_fields.find((field) => field.name === "to_user_id");
        if (userField) {
          userField.data_list = (response.data.data || []).map((item) => ({
            value: item.id,
            label: item.name,
          }));

          console.log(userField.data_list);
        }
      }
    },
  },
