                  ...
                  <tr v-for="(item, index) in all?.data" :key="item.id" :class="`table_rows table_row_${item.id}`">
                    ...
                    <td>
                      <span v-if="item.meeting_link" style="cursor: pointer">
                        ...
                        <i class="fa fa-copy ml-1 text-info" @click.stop="copyToClipboard(item.meeting_link)"></i>
                      </span>
                      <span v-else>-</span>
                    </td>
...
export default {
  ...
  methods: {
    applyDateQueryFilter() {
      // Always call set_filter_criteria, even if query param is missing
      let hasDate = Object.prototype.hasOwnProperty.call(this.$route.query, "date");
      let date = this.$route.query.date;
      if (hasDate && date !== "") {
        // console.log("[IF] Query param exists and is not empty:", date);
        this.set_filter_criteria({
          start_date: date,
          end_date: date,
        });
      } else {
        // console.log("[ELSE] Query param missing or empty", hasDate, date);
        this.set_filter_criteria({
          start_date: "",
          end_date: "",
        });
